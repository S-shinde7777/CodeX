...
function EditorPage() {
  ...
  const handleSaveFile = async () => {
    if (currentSnippetId) {
      const response = await fetch(`http://localhost:5000/api/snippets/${currentSnippetId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`
    ...
    <div className="flex h-screen bg-[#12141A] text-gray-200">
          ...
          {snippets.map((snippet) => (
            <div
              ...
              className={`p-2 rounded-lg cursor-pointer flex justify-between items-center ${
                currentSnippetId === snippet._id ? 'bg-teal-900' : 'bg-[#1f2128]'
              }`}
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="h-40 bg-[#0d0e12] border-t border-gray-800 p-3 overflow-y-auto">
          ...
          <pre className="text-sm text-teal-300 whitespace-pre-wrap">{output}</pre>
        </div>
      </div>
